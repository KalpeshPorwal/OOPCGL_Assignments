...
#include<string.h>	
#include<stdlib.h>		//header file for string class
...
class student   							//DEFINITION OF STUDENT CLASS
{
    char name[30];
	char address[50]; 
	char std[10];				//OBJECTS OF STRING CLASS
    char div;
    int roll_no;
    long mob;
...
{
        strcpy(name,"Shalaka Alkute");
        strcpy(address,"Savedi , Ahmednagar");
        strcpy(std , "SE 2 COMPUTER");
        div='A';
        roll_no=21042;
        mob=942329999;
}
...
{
     strcpy(this->name, obj.name);							//this is a pointer points to the object which invokes it
     strcpy(this->address , obj.address);	//this-> can be written as name
     strcpy(this->std , obj.std);
      this->div=obj.div;
...
int main()
{
 ...
 do
 {
 	...
 	//default : cout<<"\nPlease enter valid entry for choice , Thank you ! ";
  }while(ans=='y') 	;
